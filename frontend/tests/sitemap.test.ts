...
describe('buildSitemapXml', () => {
  it('renders a valid urlset with loc + optional lastmod', () => {
    const xml = buildSitemapXml([
      ...
      { loc: 'https://rootsandrings.net/products/kura-vessel', lastmod: '2026-06-01' },
    ]);
    expect(xml).toContain('<?xml version="1.0" encoding="UTF-8"?>');
    expect(xml).toContain('<urlset');
    expect(xml).toContain('<loc>https://rootsandrings.net/products/kura-vessel</loc>');
    expect(xml).toContain('<lastmod>2026-06-01</lastmod>');
    expect(xml.match(/<url>/g)?.length).toBe(2);
  });

  it('xml-escapes special characters in loc', () => {
    const xml = buildSitemapXml([{ loc: 'https://rootsandrings.net/products?q=a&b' }]);
